...
function etspawnbiome(entities, a) {
    ...
    if (a.spawnbiome == 4) {//defal
        if (a.biome == 4 || a.biome == 5) return;
        randomtype = Math.floor(Math.random() * arctic.length);
        a.pos.x = randomNumber(entities[arctic[randomtype]].x - entities[arctic[randomtype]].radius / 2, entities[arctic[randomtype]].x + entities[volcano[randomtype]].radius / 2)
        a.pos.y = randomNumber(entities[arctic[randomtype]].y - entities[arctic[randomtype]].radius / 2, entities[arctic[randomtype]].y + entities[volcano[randomtype]].radius / 2)
    }
}
